          ...
          <tr v-for="item in filteredLotes" :key="item.ID" :class="getRowClass(item)">
            ...
            <td class="px-6 py-4">
              ...
              <div v-else class="flex items-center">
                <button @click="changeValue(item, 'Costo_Total', -1)"
                  class="px-2 py-1 border rounded-l bg-gray-200 hover:bg-gray-300">-</button>
                <input v-model.number="item.Costo_Total" type="number" step="0.01" min="0"
            ...
            <td class="px-6 py-4 text-center">
              ...
              <a href="#" v-if="!item.editing" @click.prevent="deleteItem(item.ID)"
                class="font-medium text-red-600 hover:underline ml-2">Eliminar</a>
              <a href="#" v-if="item.editing" @click.prevent="saveItem(item)"
                class="font-medium text-green-600 hover:underline ml-2">Guardar</a>
              <a href="#" v-if="item.editing" @click.prevent="cancelEdit(item)"
                class="font-medium text-gray-600 hover:underline ml-2">Cancelar</a>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    async fetchLotes() {
      try {
        const response = await axios.get('https://privilegecare-deploy-gqmt.onrender.com/lotes/', {
          headers: {
        ...
        console.error('Error al obtener los lotes:', error.response ? error.response.data : error.message);
      }
    }
    ,
    async fetchMedicamentos() {
      try {
        ...
        console.error('Error al obtener los medicamentos:', error.response ? error.response.data : error.message);
      }
    }
    ,
    ...
    async saveItem(item) {
      ...
      try {
        const url = `https://privilegecare-deploy-gqmt.onrender.com/lote/${item.ID}/`;
    ...
    async deleteItem(id) {
      try {
        const url = `https://privilegecare-deploy-gqmt.onrender.com/lote/${id}/`;
